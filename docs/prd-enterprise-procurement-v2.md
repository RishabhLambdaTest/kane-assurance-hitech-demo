...
- **Finance Director (Second-level Approver)** — gives final approval on orders above $100,000 after the Procurement Manager has approved. *(NEW in v2)*
...
- Payment terms are Net 30 by default. Orders with a total above $50,000 may select Net 45 terms instead *(changed in v2)*
...
Orders whose total (including tax and delivery) exceeds $25,000 require approval. The approval chain depends on the order total *(changed in v2)*:

| Order total | Approval chain |
|-------------|----------------|
| ≤ $25,000 | No approval — order placed directly |
| $25,000.01 – $100,000 | Procurement Manager |
| > $100,000 | Procurement Manager, then Finance Director |
...
- For orders above $100,000, the Procurement Manager's approval changes the status to "Pending Finance Approval". The Finance Director's approval changes it to "Approved" and places the order. A rejection at either level ends the request.
- For orders up to $100,000, the Procurement Manager's approval changes the status to "Approved" and places the order, generating an order number (FR-8).
...
### FR-9: Device-as-a-Service (NEW in v2)

Buyers can acquire devices on a monthly subscription instead of purchasing outright:
- A "Buy / Subscribe" toggle appears on each hardware line in the quote. Subscribe is available for Laptop, Workstation, and Tablet lines only — never accessories
- Subscription requires a line quantity of 10 or more; below 10 the toggle is disabled with the reason "Device-as-a-Service requires 10 or more devices"
- Term is fixed at 36 months. Monthly price per device = (discounted hardware unit price + services per device) ÷ 36 × 1.08, rounded to the nearest cent
- ProSupport (3 years) is automatically included on subscription lines and cannot be removed
- Subscription lines show "$X / device / month" and a separate "Monthly recurring total" in the quote summary; they are excluded from the one-time order total and from the $25,000 approval threshold, which is instead evaluated on 36 × monthly recurring total
- Subscriptions can be paid by Purchase Order only; the corporate card option is hidden when the quote contains any subscription line
- The confirmation page shows a subscription agreement number in the format `DAAS-` followed by 6 digits and the term end date
